...
export function Setup({ onStart, onImportResults, googleAuth, onConnectGoogle, onSwitchGoogleAccount, onDisconnectGoogle, googleSheet, onCreateGoogleSheet, onPickGoogleSheet, onOpenGoogleResults, googleSheetWriteStatus, googleSheetReadStatus, interruptedSession, onOpenInterruptedSession, onDismissInterruptedSession }) {
  ...
  const inp = { flex: 1, background: "var(--color-surface, #FFFFFF)", border: "1px solid var(--color-border, #E6E2D9)", borderRadius: "6px", color: "var(--color-text, #1F1F1F)", padding: "9px 12px", fontSize: "0.88rem", fontFamily: "inherit", outline: "none", boxShadow: "0 1px 0 rgba(31, 31, 31, 0.03)" };
  ...
  const recommendedSummary = guessPolicy === GUESS_POLICIES.ONE_SHOT
    ...
    : (deckPolicy === DECK_POLICIES.BALANCED_DECK
      ? "Best for structured calibration with repeated feedback and balanced exposure."
      : "Best for exploratory calibration with repeated feedback under fully independent draws.");

  return (
    <div style={{ minHeight: "100vh", background: "var(--color-bg, #F7F6F2)", color: "var(--color-text, #1F1F1F)", fontFamily: "'Georgia', serif", display: "flex", flexDirection: "column", alignItems: "center", padding: "48px 24px" }}>
      <div style={{ position: "relative", marginBottom: "40px", textAlign: "center", padding: "28px 28px", borderRadius: "16px", background: "var(--color-surface, #FFFFFF)", border: "1px solid var(--color-border, #E6E2D9)", overflow: "hidden", boxShadow: "0 18px 40px rgba(31, 31, 31, 0.07)" }}>
        {[
          ["3%","8px","#e9d5ff","0.9","0.5rem","✦"],["8%","52px","#fff","0.5","0.16rem","·"],
          ["13%","20px","#E6E2D9","0.7","0.22rem","★"],["18%","65px","#E6E2D9","0.85","0.4rem","✦"],
          ["23%","5px","#fff","0.35","0.15rem","·"],["28%","48px","#ddd6fe","0.7","0.28rem","✦"],
          ["33%","75px","#a5b4fc","0.5","0.2rem","·"],["38%","2px","#f5d0fe","0.9","0.46rem","★"],
          ["43%","68px","#fff","0.3","0.15rem","·"],["49%","0px","#E6E2D9","0.8","0.44rem","✦"],
          ...
          ["94%","44px","#E6E2D9","0.6","0.22rem","✦"],["97%","25px","#fff","0.4","0.16rem","·"],
          ["46%","38px","#fff","0.22","0.13rem","·"],["15%","40px","#e9d5ff","0.4","0.18rem","✦"],
          ["70%","36px","#f5d0fe","0.45","0.2rem","★"],["52%","18px","#ddd6fe","0.3","0.15rem","·"],
        ].map(([l,t,c,,fs,sym],i) => (
          <div key={i} style={{ position: "absolute", left: l, top: t, color: c, opacity: 0, fontSize: fs, pointerEvents: "none", userSelect: "none", lineHeight: 1, zIndex: 0 }}>{sym}</div>
        ))}
        <div style={{ position: "relative", zIndex: 1 }}>
          <div style={{ fontFamily: "Cormorant Garamond, Georgia, serif", fontSize: "2.9rem", fontWeight: 600, letterSpacing: "0.28em", textTransform: "uppercase", color: "var(--color-primary, #2F5D50)", lineHeight: 1.1 }}>
            MINDSIGHT
          </div>
          <div style={{ fontSize: "0.78rem", letterSpacing: "0.28em", color: "var(--color-subtext, #6B6B6B)", textTransform: "uppercase", marginTop: "6px" }}>ROUND SETUP</div>
        </div>
      </div>
      <div style={{ width: "100%", maxWidth: "420px", display: "flex", flexDirection: "column", gap: "32px" }}>
        <section style={{ ...sectionCardStyle, alignItems: "center" }}>
          <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: "12px" }}>
            <div style={{ display: "flex", gap: "8px", width: "100%" }}>
              <button
                ...
                style={{
                  ...
                  justifyContent: "center",
                  gap: "9px",
                  background: utilitySource === "google" ? "#E7F0EC" : "transparent",
                  border: utilitySource === "google" ? "1px solid rgba(47, 93, 80, 0.35)" : "1px solid var(--color-border, #E6E2D9)",
                  borderRadius: "10px",
                  color: utilitySource === "google" ? "var(--color-primary, #2F5D50)" : "var(--color-subtext, #6B6B6B)",
                  padding: "10px 12px",
                  minHeight: "44px",
                  fontSize: "0.76rem",
                  fontFamily: "\"Google Sans\", \"Segoe UI\", Arial, sans-serif",
              ...
              <button
                ...
                style={{
                  flex: 1,
                  background: utilitySource === "csv" ? "#EEF4F1" : "transparent",
                  border: utilitySource === "csv" ? "1px solid rgba(47, 93, 80, 0.32)" : "1px solid var(--color-border, #E6E2D9)",
                  borderRadius: "10px",
                  color: utilitySource === "csv" ? "var(--color-primary, #2F5D50)" : "var(--color-subtext, #6B6B6B)",
                  padding: "10px 12px",
                  minHeight: "44px",
                  fontSize: "0.76rem",
                      ...
                      <>
                        <button
                          ...
                          style={{
                            ...
                            background: googleConfigured ? "#ffffff" : "var(--color-surface-soft, #FBFAF7)",
                            border: googleConfigured ? "1px solid #d1d5db" : "1px solid var(--color-border, #E6E2D9)",
                            color: googleConfigured ? "#111827" : "var(--color-subtext, #6B6B6B)",
                        ...
                        <div style={{ fontSize: "0.66rem", color: googleConfigured ? "var(--color-subtext, #6B6B6B)" : "var(--color-error, #A94442)", letterSpacing: "0.04em", lineHeight: 1.6, textAlign: "center" }}>
                ...
                {googleAuth?.status === "connected" && (
                  <>
                    {googleSheet?.status === "selected" && (
                      <div style={{ background: "var(--color-surface-soft, #FBFAF7)", border: "1px solid var(--color-border, #E6E2D9)", borderRadius: "10px", padding: "12px 14px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "12px" }}>
                        ...
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          style={{ fontSize: "0.76rem", color: "var(--color-primary, #2F5D50)", lineHeight: 1.5, textDecoration: "none", flex: 1, textAlign: "right" }}
                        >
                    ...
                    <div style={{ display: "flex", gap: "10px" }}>
                      <button
                        ...
                        style={{ flex: 0.95, background: googleSheet?.status === "creating" ? "var(--color-surface-soft, #FBFAF7)" : "transparent", border: "1px solid rgba(58, 125, 68, 0.35)", borderRadius: "10px", color: googleSheet?.status === "creating" ? "var(--color-subtext, #6B6B6B)" : "var(--color-success, #3A7D44)", padding: "12px", minHeight: "48px", fontSize: "0.75rem", fontFamily: "\"Segoe UI\", Arial, sans-serif", letterSpacing: "0.05em", textTransform: "none", cursor: googleSheet?.status === "creating" ? "wait" : "pointer", transition: "all 0.2s" }}
                      >
                      ...
                      <button
                        ...
                        style={{ flex: 1.05, background: !googlePickerConfigured || googleSheet?.status === "selecting" ? "var(--color-surface-soft, #FBFAF7)" : "var(--color-primary, #2F5D50)", border: !googlePickerConfigured ? "1px solid var(--color-border, #E6E2D9)" : "1px solid var(--color-primary, #2F5D50)", borderRadius: "10px", color: !googlePickerConfigured ? "var(--color-subtext, #6B6B6B)" : googleSheet?.status === "selecting" ? "var(--color-subtext, #6B6B6B)" : "#FFFFFF", padding: "12px", minHeight: "48px", fontSize: "0.76rem", fontFamily: "\"Segoe UI\", Arial, sans-serif", letterSpacing: "0.05em", textTransform: "none", cursor: !googlePickerConfigured ? "not-allowed" : googleSheet?.status === "selecting" ? "wait" : "pointer", transition: "all 0.2s", boxShadow: !googlePickerConfigured || googleSheet?.status === "selecting" ? "none" : "0 10px 24px rgba(47, 93, 80, 0.18)" }}
                      >
                    ...
                    <button
                      ...
                      style={{ background: googleSheet?.status === "selected" ? "var(--color-primary, #2F5D50)" : "var(--color-surface-soft, #FBFAF7)", border: googleSheet?.status === "selected" ? "1px solid var(--color-primary, #2F5D50)" : "1px solid var(--color-border, #E6E2D9)", borderRadius: "10px", color: googleSheet?.status === "selected" ? "#FFFFFF" : "var(--color-subtext, #6B6B6B)", padding: "12px", minHeight: "48px", fontSize: "0.76rem", fontFamily: "\"Segoe UI\", Arial, sans-serif", letterSpacing: "0.05em", textTransform: "none", cursor: googleSheet?.status === "selected" ? "pointer" : "not-allowed", transition: "all 0.2s", boxShadow: googleSheet?.status === "selected" ? "0 10px 24px rgba(47, 93, 80, 0.18)" : "none" }}
                    >
                      Open Google Results
                    </button>
                ...
                <div style={{ display: "flex", gap: "10px", width: "100%" }}>
                  <div style={{ flex: 1 }}>
                    <CsvImportButton
                      ...
                      buttonStyle={{ width: "100%", background: "var(--color-surface-soft, #FBFAF7)", border: "1px solid var(--color-border, #E6E2D9)", borderRadius: "10px", color: "var(--color-primary, #2F5D50)", padding: "12px", minHeight: "48px", fontSize: "0.82rem", fontFamily: "\"Segoe UI\", Arial, sans-serif", letterSpacing: "0.06em", textTransform: "none", cursor: "pointer", transition: "all 0.2s" }}
                  ...
                  <div style={{ flex: 1 }}>
                    <CsvImportButton
                      ...
                      buttonStyle={{ width: "100%", background: "var(--color-primary, #2F5D50)", border: "1px solid var(--color-primary, #2F5D50)", borderRadius: "10px", color: "#FFFFFF", padding: "12px", minHeight: "48px", fontSize: "0.82rem", fontFamily: "\"Segoe UI\", Arial, sans-serif", letterSpacing: "0.05em", textTransform: "none", cursor: "pointer", transition: "all 0.2s", boxShadow: "0 10px 24px rgba(47, 93, 80, 0.18)" }}
                      statusStyle={{ display: "none" }}
                    />
                  </div>
                </div>
            ...
            {utilitySource === "google" && googleStatusMessage && (
              <div style={{ width: "100%", fontSize: "0.68rem", color: googleAuth?.error || googleSheet?.error ? "var(--color-error, #A94442)" : "var(--color-success, #3A7D44)", letterSpacing: "0.04em", lineHeight: 1.6, textAlign: "center" }}>
                {googleStatusMessage}
              </div>
            )}
            {utilitySource === "csv" && csvStatusMessage && (
              <div style={{ width: "100%", fontSize: "0.68rem", color: importError ? "var(--color-error, #A94442)" : "var(--color-success, #3A7D44)", letterSpacing: "0.04em", lineHeight: 1.6, textAlign: "center" }}>
                {csvStatusMessage}
              </div>
            )}

            {interruptedSession && (
              <div style={{ width: "100%", background: "var(--color-surface-soft, #FBFAF7)", border: "1px solid rgba(197, 139, 43, 0.38)", borderRadius: "12px", padding: "12px 14px", display: "flex", flexDirection: "column", gap: "10px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: "10px", flexWrap: "wrap", alignItems: "baseline" }}>
                  ...
                  <div style={{ fontSize: "0.68rem", color: "var(--color-subtext, #6B6B6B)", letterSpacing: "0.06em" }}>
                    {interruptedSession?.endedAt ? `Ended ${new Date(interruptedSession.endedAt).toLocaleString("en-US")}` : "Ended recently"}
                  </div>
                </div>
                <div style={{ fontSize: "0.72rem", color: "var(--color-text, #1F1F1F)", lineHeight: 1.6 }}>
                ...
                <div style={{ display: "flex", gap: "10px", justifyContent: "flex-end", flexWrap: "wrap" }}>
                  <button
                    ...
                    style={{ background: "transparent", border: "1px solid var(--color-border, #E6E2D9)", borderRadius: "10px", color: "var(--color-subtext, #6B6B6B)", padding: "10px 14px", fontSize: "0.76rem", fontFamily: "\"Segoe UI\", Arial, sans-serif", letterSpacing: "0.05em", cursor: "pointer" }}
                  >
                    Dismiss
                  </button>
                  <button
                    onClick={onOpenInterruptedSession}
                    style={{ background: "var(--color-primary, #2F5D50)", border: "none", borderRadius: "10px", color: "white", padding: "10px 16px", fontSize: "0.76rem", fontFamily: "\"Segoe UI\", Arial, sans-serif", letterSpacing: "0.05em", cursor: "pointer", boxShadow: "0 10px 24px rgba(47, 93, 80, 0.18)" }}
                  >
                    Open Results
                  </button>
                </div>
        ...
        <section style={sectionCardStyle}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "14px", flexWrap: "wrap" }}>
            <div style={{ fontSize: "0.82rem", color: "var(--color-text, #1F1F1F)", letterSpacing: "0.16em", textTransform: "uppercase", fontWeight: 700, whiteSpace: "nowrap" }}>
              Session Mode
            </div>
            <select value={appMode} onChange={(e) => { setAppMode(e.target.value); clearPreviewMemory(); }} style={{ ...modeSelectStyle, minWidth: "190px", marginLeft: "auto", flexShrink: 0 }}>
              <option value={SESSION_MODES.SOLO} style={{ background: "#FFFFFF", color: "#1F1F1F", textAlign: "center", fontFamily: "'Cormorant Garamond', Georgia, serif" }}>Solo Calibration</option>
              <option value={SESSION_MODES.SHARED} style={{ background: "#FFFFFF", color: "#1F1F1F", textAlign: "center", fontFamily: "'Cormorant Garamond', Georgia, serif" }}>Shared Calibration</option>
              <option value={SESSION_MODES.GROUP} style={{ background: "#FFFFFF", color: "#1F1F1F", textAlign: "center", fontFamily: "'Cormorant Garamond', Georgia, serif" }}>Group Tracker</option>
            </select>
          </div>
          {appMode === SESSION_MODES.SOLO ? (
            <div>
            ...
            <div>
              ...
              {names.map((n, i) => (
                <div key={i} style={{ display: "flex", gap: "8px", marginBottom: "7px", alignItems: "center" }}>
                  <span style={{ color: "var(--color-subtext, #6B6B6B)", fontSize: "0.75rem", width: "16px", textAlign: "right", flexShrink: 0 }}>{i+1}</span>
              ...
              {names.length < 10 && <div style={{display:"flex",justifyContent:"center"}}><button onClick={addP} style={{ marginTop: "16px", width: "fit-content", background: "var(--color-surface-soft, #FBFAF7)", border: "1px solid var(--color-border, #E6E2D9)", borderRadius: "6px", color: "var(--color-primary, #2F5D50)", padding: "7px 16px", cursor: "pointer", fontSize: "0.78rem", fontFamily: "inherit", letterSpacing: "0.06em" }}>+ Add Participant</button></div>}
            </div>
          )}
        </section>

        <section style={sectionCardStyle}>
          <div style={{ fontSize: "0.78rem", color: "var(--color-subtext, #6B6B6B)", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "10px", fontWeight: 500, textAlign: "left" }}>Category</div>
          <div style={{ display: "flex", gap: "8px", marginBottom: "16px", flexWrap: "wrap" }}>
            {Object.keys(CATEGORIES).map(cat => {
              const isActive = category === cat;
              const styles = {
                Colors: { bg: "linear-gradient(135deg, #dc2626 0%, #ea580c 20%, #ca8a04 40%, #16a34a 60%, #2563eb 80%, #9333ea 100%)", border: "1.5px solid rgba(255,255,255,0.35)", color: "white", shadow: "0 0 0 1px rgba(255,255,255,0.15), 0 4px 16px rgba(31,31,31,0.22)", textShadow: "0 1px 4px rgba(31,31,31,0.5)" },
                Numbers: { bg: "#FFF4E3", border: "1.5px solid #C58B2B", color: "#8A5E17", shadow: "0 4px 16px rgba(197,139,43,0.16)", textShadow: "none" },
              ...
              return (
                <button key={cat} onClick={() => switchCategory(cat)} style={{ position: "relative", background: isActive ? s.bg : "var(--color-surface-soft, #FBFAF7)", border: isActive ? s.border : "1.5px solid var(--color-border, #E6E2D9)", borderRadius: "10px", padding: "11px 22px", cursor: "pointer", color: isActive ? s.color : "var(--color-subtext, #6B6B6B)", fontSize: "0.88rem", fontFamily: cat==="Numbers" ? "Georgia, serif" : "inherit", fontWeight: isActive ? 600 : 400, letterSpacing: "0.06em", transition: "all 0.15s", boxShadow: isActive ? s.shadow : "none", textShadow: isActive ? s.textShadow : "none", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginTop: "8px", marginBottom: "6px" }}>
            <div style={{ fontSize: "0.72rem", color: "var(--color-subtext, #6B6B6B)", letterSpacing: "0.15em", textTransform: "uppercase", fontWeight: 500 }}>Active {CATEGORIES[category].label}</div>
            <div style={{ display: "flex", gap: "6px" }}>
              ...
              }} style={{ fontSize: "0.65rem", color: "var(--color-primary, #2F5D50)", background: "transparent", border: "1px solid var(--color-border, #E6E2D9)", borderRadius: "4px", padding: "3px 8px", cursor: "pointer", fontFamily: "inherit", letterSpacing: "0.05em" }}>All</button>
              <button onClick={() => {
              ...
              }} style={{ fontSize: "0.65rem", color: "var(--color-primary, #2F5D50)", background: "transparent", border: "1px solid var(--color-border, #E6E2D9)", borderRadius: "4px", padding: "3px 8px", cursor: "pointer", fontFamily: "inherit", letterSpacing: "0.05em" }}>None</button>
            </div>
          </div>
          <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
            {CATEGORIES[category].items.map(c => {
              ...
              return (
                <button key={c.name} title={on && enabled.size <= 2 ? "Minimum 2 active items, 2 rounds" : c.name} onClick={() => toggleItem(c.name)} style={{ background: on ? "var(--color-surface-soft, #FBFAF7)" : "transparent", border: `2px solid ${on ? c.hex : "var(--color-border, #E6E2D9)"}`, borderRadius: "8px", padding: "7px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: "6px", opacity: on ? 1 : 0.7, transition: "all 0.12s", fontFamily: "inherit", position: "relative" }}>
                  <span style={{ fontSize: "1.1rem", lineHeight: 1, color: on ? c.hex : "#888", filter: on ? `drop-shadow(0 0 4px ${c.hex}44)` : "none" }}>{c.symbol}</span>
                  <span style={{ fontSize: "0.78rem", color: on ? c.hex : "var(--color-subtext, #6B6B6B)" }}>{c.name}</span>
                  {on && preview && (
                    <span style={{ position: "absolute", top: "-10px", right: "-10px", background: preview.counts[c.name] > 0 ? c.hex : "var(--color-border, #E6E2D9)", color: preview.counts[c.name] > 0 ? "white" : "var(--color-subtext, #6B6B6B)", borderRadius: "99px", fontSize: "0.7rem", fontWeight: 700, minWidth: "22px", height: "22px", display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px", lineHeight: 1, border: "2px solid var(--color-surface, #FFFFFF)", boxShadow: preview.counts[c.name] > 0 ? `0 0 6px ${c.hex}66` : "none" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", marginTop: "12px" }}>
            <div style={{ fontSize: "0.82rem", color: "var(--color-text, #1F1F1F)", letterSpacing: "0.16em", textTransform: "uppercase", fontWeight: 600, whiteSpace: "nowrap" }}>
              Cards per Round
            </div>
        ...
        <section style={{ ...sectionCardStyle, gap: "12px" }}>
          <div style={{ fontSize: "0.72rem", color: "var(--color-subtext, #6B6B6B)", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "4px", fontWeight: 500 }}>Guess Policy</div>
          <div style={{ display: "flex", gap: "10px", marginBottom: "6px" }}>
            <button onClick={() => setGuessPolicy(GUESS_POLICIES.REPEAT_UNTIL_CORRECT)} style={{ flex: "1 1 0", minWidth: 0, background: guessPolicy===GUESS_POLICIES.REPEAT_UNTIL_CORRECT ? "#FFF4E3" : "transparent", border: `2px solid ${guessPolicy===GUESS_POLICIES.REPEAT_UNTIL_CORRECT ? "var(--color-warning, #C58B2B)" : "var(--color-border, #E6E2D9)"}`, borderRadius: "8px", padding: "10px 12px", cursor: "pointer", color: guessPolicy===GUESS_POLICIES.REPEAT_UNTIL_CORRECT ? "var(--color-warning, #C58B2B)" : "var(--color-subtext, #6B6B6B)", fontSize: "0.8rem", fontFamily: "inherit", transition: "all 0.12s", textAlign: "center", letterSpacing: "0.02em" }}>Repeat Until Correct</button>
            <button onClick={() => setGuessPolicy(GUESS_POLICIES.ONE_SHOT)} style={{ flex: "1 1 0", minWidth: 0, background: guessPolicy===GUESS_POLICIES.ONE_SHOT ? "#E7F0EC" : "transparent", border: `2px solid ${guessPolicy===GUESS_POLICIES.ONE_SHOT ? "var(--color-primary, #2F5D50)" : "var(--color-border, #E6E2D9)"}`, borderRadius: "8px", padding: "10px 12px", cursor: "pointer", color: guessPolicy===GUESS_POLICIES.ONE_SHOT ? "var(--color-primary, #2F5D50)" : "var(--color-subtext, #6B6B6B)", fontSize: "0.8rem", fontFamily: "inherit", transition: "all 0.12s", textAlign: "center", letterSpacing: "0.02em" }}>One Shot</button>
          </div>

          <div style={{ fontSize: "0.72rem", color: "var(--color-subtext, #6B6B6B)", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "4px", fontWeight: 500 }}>Deck Policy</div>
          <div style={{ display: "flex", gap: "10px" }}>
            <button onClick={() => { setDeckPolicy(DECK_POLICIES.BALANCED_DECK); clearPreviewMemory(); }} style={{ flex: "1 1 0", minWidth: 0, background: deckPolicy===DECK_POLICIES.BALANCED_DECK ? "#FFF4E3" : "transparent", border: `2px solid ${deckPolicy===DECK_POLICIES.BALANCED_DECK ? "var(--color-warning, #C58B2B)" : "var(--color-border, #E6E2D9)"}`, borderRadius: "8px", padding: "10px 12px", cursor: "pointer", color: deckPolicy===DECK_POLICIES.BALANCED_DECK ? "var(--color-warning, #C58B2B)" : "var(--color-subtext, #6B6B6B)", fontSize: "0.8rem", fontFamily: "inherit", transition: "all 0.12s", textAlign: "center", letterSpacing: "0.02em" }}>Balanced Deck</button>
            <button onClick={() => { setDeckPolicy(DECK_POLICIES.INDEPENDENT_DRAWS); clearPreviewMemory(); }} style={{ flex: "1 1 0", minWidth: 0, background: deckPolicy===DECK_POLICIES.INDEPENDENT_DRAWS ? "#E7F0EC" : "transparent", border: `2px solid ${deckPolicy===DECK_POLICIES.INDEPENDENT_DRAWS ? "var(--color-primary, #2F5D50)" : "var(--color-border, #E6E2D9)"}`, borderRadius: "8px", padding: "10px 12px", cursor: "pointer", color: deckPolicy===DECK_POLICIES.INDEPENDENT_DRAWS ? "var(--color-primary, #2F5D50)" : "var(--color-subtext, #6B6B6B)", fontSize: "0.8rem", fontFamily: "inherit", transition: "all 0.12s", textAlign: "center", letterSpacing: "0.02em" }}>Independent Draws</button>
          </div>

          <button onClick={runPreview} disabled={!canPreview} style={{ marginTop: "10px", background: canPreview ? "var(--color-primary, #2F5D50)" : "var(--color-surface-soft, #FBFAF7)", border: canPreview ? "1px solid var(--color-primary, #2F5D50)" : "1px solid var(--color-border, #E6E2D9)", borderRadius: "8px", color: canPreview ? "#FFFFFF" : "var(--color-subtext, #6B6B6B)", padding: "12px", fontSize: "0.82rem", fontFamily: "inherit", letterSpacing: "0.1em", textTransform: "uppercase", cursor: canPreview ? "pointer" : "not-allowed", transition: "all 0.2s" }}>
            {preview ? "Re-roll Deck" : "Preview Deck"}
          </button>
          <div style={{ fontSize: "0.68rem", color: "var(--color-subtext, #6B6B6B)", letterSpacing: "0.04em", lineHeight: 1.6 }}>
            {deckPolicy === DECK_POLICIES.BALANCED_DECK
            ...
            {preview && <span style={{ color: "var(--color-primary, #2F5D50)" }}> · {slots} cards locked in.</span>}
          </div>
          <div style={{ background: "var(--color-surface-soft, #FBFAF7)", border: "1px solid var(--color-border, #E6E2D9)", borderLeft: "3px solid var(--color-primary, #2F5D50)", borderRadius: "10px", padding: "12px 14px", display: "flex", flexDirection: "column", gap: "8px" }}>
            <div style={{ fontSize: "0.68rem", color: "var(--color-subtext, #6B6B6B)", letterSpacing: "0.12em", textTransform: "uppercase" }}>Mode Summary</div>
            <div style={{ fontSize: "0.76rem", color: "var(--color-text, #1F1F1F)", lineHeight: 1.6 }}>{guessPolicySummary}</div>
            <div style={{ fontSize: "0.76rem", color: "var(--color-primary, #2F5D50)", lineHeight: 1.6 }}>{deckPolicySummary}</div>
            <div style={{ fontSize: "0.72rem", color: "var(--color-subtext, #6B6B6B)", lineHeight: 1.6 }}>{recommendedSummary}</div>
          </div>
        </section>

        <section style={{ ...sectionCardStyle, gap: "12px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: "14px", flexWrap: "wrap" }}>
            <div style={{ fontSize: "0.78rem", color: "var(--color-subtext, #6B6B6B)", letterSpacing: "0.14em", textTransform: "uppercase", fontWeight: 600 }}>
              Session ID
            </div>
            {resumeIndex > 0 && (
          ...
          <button onClick={generateShareSession} disabled={!preview} style={{ background: preview ? "transparent" : "var(--color-surface-soft, #FBFAF7)", border: preview ? "1px solid var(--color-primary, #2F5D50)" : "1px solid var(--color-border, #E6E2D9)", borderRadius: "8px", color: preview ? "var(--color-primary, #2F5D50)" : "var(--color-subtext, #6B6B6B)", padding: "12px", fontSize: "0.82rem", fontFamily: "inherit", letterSpacing: "0.1em", textTransform: "uppercase", cursor: preview ? "pointer" : "not-allowed", transition: "all 0.2s" }}>
            Generate Session ID
          </button>
          {sharedCodeError && (
            <div style={{ fontSize: "0.68rem", color: "var(--color-error, #A94442)", letterSpacing: "0.04em", lineHeight: 1.6 }}>
              {sharedCodeError}
            </div>
          )}
          <div style={{ fontSize: "0.68rem", color: "var(--color-subtext, #6B6B6B)", letterSpacing: "0.04em", lineHeight: 1.6 }}>
            This session code can restore the current setup and deck across solo calibration, shared calibration, and group tracker.
          </div>
        </section>

          <div style={{ display: "flex", flexDirection: "column", gap: "10px", marginTop: "8px" }}>
            <button onClick={go} disabled={!canStart || !preview} style={{ background: (canStart && preview) ? "var(--color-primary, #2F5D50)" : "var(--color-surface-soft, #FBFAF7)", border: (canStart && preview) ? "none" : "1px solid var(--color-border, #E6E2D9)", borderRadius: "8px", color: (canStart && preview) ? "white" : "var(--color-subtext, #6B6B6B)", padding: "15px", fontSize: "0.95rem", fontFamily: "Cormorant Garamond, Georgia, serif", letterSpacing: "0.16em", textTransform: "uppercase", cursor: (canStart && preview) ? "pointer" : "not-allowed", boxShadow: (canStart && preview) ? "0 12px 30px rgba(47, 93, 80, 0.24)" : "none", transition: "all 0.2s" }}>
              {appMode === SESSION_MODES.GROUP ? "Start Round →" : isResumeSession ? "Resume Calibration →" : "Begin Calibration →"}
            </button>
          </div>
      </div>
    </div>
